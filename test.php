...
<head>
  ...
  $batas = 10;
  #$input_batas=$_GET['batas'];
  $halaman = isset($_GET['halaman']) ? (int)$_GET['halaman'] : 1;
  #if($input_batas==0){
    $halaman_awal = ($halaman > 1) ? ($halaman * $batas) - $batas : 0;
#}
  #else{
    #$halaman_awal = ($halaman > 1) ? ($halaman * $input_batas) - $input_batas : 0;
  #}
...
if (!empty($_GET)) {
    ...
    #else{
            #$a = $_GET["search"];
            #$sql = "SELECT * FROM user WHERE name LIKE '%$a%' OR email LIKE '%$a%' OR phone LIKE '%$a%' LIMIT $halaman_awal, $input_batas";
            #$result = $conn->query($sql);
        #}
}
else {
    ...
    #else{
        #$a = '';
        #$sql = "SELECT * FROM user LIMIT $halaman_awal, $input_batas";
        #$result = $conn->query($sql);
    #}
}
  ...
  #}
  #else{
    #$total_halaman = ceil($jumlah_data / $input_batas);
  #}
              ...
              <div class="row mb-3">
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="get">
                  <div class="row col-12 mb-2">
                    ...
                    <div class="col">
                        <input type="text" id="batas" name="batas" placeholder="Masukan Jumlah data per halaman" class="form-control" title="Enter search keyword">
                    </div>
                    <div class="col">
                        <input type="text" id="halaman" name="halaman" placeholder="Halaman yang dituju" class="form-control" title="Enter search keyword">
                    </div>
                    <div class="col-2 d-flex justify-content-end align-items-center">
                    <input type="submit" name="custom" class="button" value="go">
                    <?php if(array_key_exists('custom')) {
                    custom();
                    }?>
                    </div>
